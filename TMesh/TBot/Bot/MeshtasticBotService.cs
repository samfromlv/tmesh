        ...
        private async Task ProcessInboundPrivateChannelMeshTextMessage(TextMessage message, Device deviceOrNull)
        {
            ...
            if (message.ReplyTo != 0)
            {
                        ...
                        && chatIds.Any(x => x == replyToStatus.TelegramChatId))
                {
                    ...
                    meshSender.StoreTelegramMessageStatus(replyToStatus.TelegramChatId,
                        msg.Id,
                        status);

                    botCache.StoreMeshMessageStatus(message.Id, status);
                    sentReply = true;
                }
            }
            ...
            if (!sentReply)
            {
                foreach (var chatId in chatIds)
                {
                    ...
                    meshSender.StoreTelegramMessageStatus(chatId, msg.Id, status);
                    botCache.StoreMeshMessageStatus(message.Id, status);
                }
            }
